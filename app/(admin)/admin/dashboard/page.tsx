  ...
  return (
    <AdminLayout title="Dashboard">
      <main className="space-y-6 p-4 sm:p-6">
        ...
        <section aria-labelledby="strategic-kpis"><div className="mb-3 flex items-end justify-between"><div><h2 id="strategic-kpis" className="text-lg font-bold">Strategic KPIs</h2><p className="text-sm text-muted-foreground">Executive outcomes and 30-day growth context</p></div></div><div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-6">{kpis.map(({ label, value, detail, icon: Icon, href, delta }) => <Link key={label} href={href} className="group rounded-2xl border border-border bg-card p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/40 sm:p-4"><div className="flex items-center justify-between"><span className="rounded-xl bg-primary/10 p-2 text-primary sm:p-2.5"><Icon className="h-4 w-4 sm:h-5 sm:w-5" /></span>{delta !== undefined && <span className={`inline-flex items-center gap-1 text-xs font-bold ${delta >= 0 ? "text-emerald-600" : "text-red-600"}`}>{delta >= 0 ? <TrendingUp className="h-3.5 w-3.5" /> : <TrendingDown className="h-3.5 w-3.5" />}{delta >= 0 ? "+" : ""}{delta}%</span>}</div><p className="mt-3 text-xl font-bold sm:mt-4 sm:text-2xl">{value}</p><p className="mt-1 text-xs font-medium">{label}</p><p className="mt-0.5 text-[11px] text-muted-foreground">{detail}</p></Link>)}</div></section>

        <section aria-labelledby="operations-health"><h2 id="operations-health" className="mb-3 text-lg font-bold">Operational health</h2><div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">{operational.map(({ label, value, icon: Icon, href, tone }) => <Link key={label} href={href} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3 hover:border-primary/40 sm:p-4"><span className={`rounded-lg p-2 ${tone}`}><Icon className="h-4 w-4" /></span><div className="min-w-0"><p className="text-lg font-bold sm:text-xl">{value}</p><p className="truncate text-[11px] text-muted-foreground">{label}</p></div></Link>)}</div></section>
