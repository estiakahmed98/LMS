...
import {
  cohortToday,
  cohortEndDateMin,
  validateNewCohortDates,
} from "@/lib/cohort-dates";
...
export default function AdminCohortsPage() {
  ...
  const learnerCount = cohorts.reduce(
    (total, item) => total + item.memberCount,
    0,
  );
  const courseCount = cohorts.reduce(
    (total, item) => total + item.courseCount,
    0,
  );
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setEditorOpen(false);
      setNotice(
        `${cohort.name} created. Add courses and learners before activation.`,
      );
    } catch (error) {
      setCreateError(
      ...
      <main className="space-y-6 p-4 sm:p-6">
          ...
          <div className="relative grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                Cohorts keep course access, learner intake and assessment
                targeting aligned without repetitive enrollment work.
              </p>
            </div>
            {can("COURSES", "create") && (
              <button
              ...
              >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[10px] font-black ${statusClass(cohort.status)}`}
                    >
                      {cohort.status}
                    </span>
                  </div>
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-border pt-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1.5">
                      ...
                      {dateLabel(cohort.startDate)} -{" "}
                      {dateLabel(cohort.endDate)}
                    </span>
        ...
        {editorOpen && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="cohort-editor-title"
          >
            <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-border bg-card p-6 shadow-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2
                    id="cohort-editor-title"
                    className="mt-1 text-2xl font-black"
                  >
                    Create cohort
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Create as draft, configure courses and learners, then
                    activate.
                  </p>
                </div>
                <button
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <label className="grid gap-1.5 text-sm font-semibold">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      setDraft((current) => ({
                        ...current,
                        code: normalizeCohortCode(event.target.value),
                      }));
                    }}
                    placeholder="BOED-2026-B01"
                ...
                <label className="grid gap-1.5 text-sm font-semibold">
                  ...
                  <input
                    type="date"
                    min={cohortToday(draft.timezone)}
                ...
                <label className="grid gap-1.5 text-sm font-semibold">
                  ...
                  <input
                    type="date"
                    min={cohortEndDateMin(
                      draft.startDate,
                      cohortToday(draft.timezone),
                    )}
                    value={draft.endDate ?? ""}
                    onChange={(event) =>
                      setDraft((current) => ({
                ...
                <label className="grid gap-1.5 text-sm font-semibold sm:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Purpose, intake rules or delivery notes"
                    className="resize-none rounded-xl border border-border bg-background px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-primary/30"
                  />
                </label>
              </div>
              {createError && (
