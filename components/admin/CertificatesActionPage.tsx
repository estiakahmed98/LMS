...
export default function CertificatesActionPage() {
  ...
  const [font, setFont] = useState("Serif Formal")
  const [notice, setNotice] = useState("Ready")

  function updateCertificate(id: string, patch: Partial<Certificate>, message: string) {
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="overflow-hidden rounded-lg border border-border bg-card">
            ...
            <table className="w-full min-w-[720px]">
              <thead className="border-b border-border bg-muted/70">
                <tr>{["Cert ID", "Student", "Course", "Status", "Actions"].map((heading) => <th key={heading} className="px-4 py-3 text-left text-xs font-semibold uppercase text-muted-foreground">{heading}</th>)}</tr>
              </thead>
                  ...
                  <tr key={row.id}>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex gap-2">
                        <button onClick={() => setNotice(`Downloaded ${row.id} in mock mode.`)} className="rounded-lg border border-border p-2 hover:bg-muted" aria-label="Download certificate"><Download className="h-4 w-4" /></button>
                        <button onClick={() => updateCertificate(row.id, { status: "Revoked" }, `${row.id} revoked.`)} className="rounded-lg border border-border p-2 hover:bg-muted" aria-label="Revoke certificate"><XCircle className="h-4 w-4" /></button>
                        <button onClick={() => updateCertificate(row.id, { status: "Valid" }, `${row.id} re-issued.`)} className="rounded-lg border border-border p-2 hover:bg-muted" aria-label="Reissue certificate"><RotateCcw className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <aside className="space-y-4">
              ...
              <div className="mt-4 space-y-3">
                ...
                <button
                  onClick={() => {
                    ...
                    setNotice("Bulk issue added one mock certificate.")
                  }}
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason for revocation" rows={3} className="mt-4 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm" />
              <button
                onClick={() => {
                  if (!reason.trim()) {
                    setNotice("Revocation reason required.")
                    return
                  }
                  updateCertificate(rows[0].id, { status: "Revoked" }, `${rows[0].id} revoked with reason.`)
                  setReason("")
                }}
        ...
        <section className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          <div className="rounded-lg border border-border bg-card p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              <button onClick={() => setNotice("Director signature uploaded in mock mode.")} className="flex w-full items-center justify-center gap-2 rounded-lg border border-border px-3 py-2.5 text-sm font-semibold hover:bg-muted">
                <Upload className="h-4 w-4" />
                Director Signature Upload
              </button>
              <button onClick={() => setNotice("Official seal uploaded in mock mode.")} className="flex w-full items-center justify-center gap-2 rounded-lg border border-border px-3 py-2.5 text-sm font-semibold hover:bg-muted">
                <Upload className="h-4 w-4" />
              ...
              <select value={font} onChange={(event) => setFont(event.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm">
                <option>Serif Formal</option>
                <option>Sans Modern</option>
              </select>
              <button onClick={() => setNotice("Certificate template saved.")} className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2.5 text-sm font-semibold text-primary-foreground">
          ...
          <div className="rounded-lg border-4 bg-card p-10 text-center" style={{ borderColor }}>
            ...
            <p className="mx-auto mt-6 max-w-xl text-lg text-muted-foreground">has successfully completed Community Paramedic Training</p>
