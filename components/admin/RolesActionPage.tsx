...
export default function RolesActionPage() {
  ...
  function togglePermission(module: string, index: number) {
    setMatrix((current) =>
      current.map((row) =>
        row.module === module
          ? { ...row, values: row.values.map((value, valueIndex) => (valueIndex === index ? (value === "yes" ? "no" : "yes") : value)) }
          : row,
      ),
    )
      ...
      <div className="grid gap-6 p-6 xl:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="rounded-lg border border-border bg-card p-5">
          ...
          <div className="space-y-2">
            {roles.map((role) => (
              <button key={role} onClick={() => setActiveRole(role)} className={`flex w-full items-center gap-3 rounded-lg border px-3 py-3 text-left text-sm font-semibold ${activeRole === role ? "border-primary bg-primary/5 text-primary" : "border-border hover:bg-muted"}`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px]">
                ...
                <tbody className="divide-y divide-border">
                  {matrix.map((row) => (
                    <tr key={row.module}>
                      <td className="px-4 py-4 text-sm font-semibold">{row.module}</td>
                      {row.values.map((value, index) => (
                        <td key={`${row.module}-${columns[index]}`} className="px-4 py-4 text-center">
                          <button onClick={() => togglePermission(row.module, index)} className={`inline-flex h-8 w-8 items-center justify-center rounded-lg border ${value === "yes" ? "border-green-200 bg-green-50 text-green-700" : "border-border bg-background text-muted-foreground"}`}>
