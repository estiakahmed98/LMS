...
export default function SubmissionDetailPage({
  ...
  async function exportSubmissionPdf() {
    ...
    try {
      ...
      doc.text(
        `Status: ${humanizeStatus(activeSubmission.manualReviewStatus)}`,
        width - 14,
        17,
        { align: "right" },
      );
      ...
      doc.text(displayScore(activeSubmission), width - 14, 26, {
        align: "right",
      });
          ...
          activeSubmission.questions.length > 0
            ...
            : [
                [
                  ...
                  activeSubmission.answerPayload?.notes ??
                    "See submitted attachment(s)",
                  activeSubmission.makerTotalMarks ?? "—",
                  activeSubmission.checkerTotalMarks ?? "—",
                  activeSubmission.checkerComment ??
                    activeSubmission.makerComment ??
                    "—",
                ],
              ],
        theme: "grid",
      ...
      const safeName = data.learner.name
        ...
        .replace(/^-|-$/g, "");
      doc.save(
        `submission-${safeName || data.learner.id}-${activeSubmission.id}.pdf`,
      );
    } catch (caught) {
      setError(
                ...
                <aside className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                  <div className="border-b border-border px-5 py-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Only submissions from this learner
                    </p>
                  </div>
                    ...
                    {data.submissions.map((submission) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span
                            className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold ${statusClass(submission.manualReviewStatus)}`}
                          >
              ...
              <div className="min-w-0 space-y-5">
                <section className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(activeSubmission.manualReviewStatus)}`}
                        >
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        ...
                        {activeSubmission.assessmentType}
                      </p>
                    </div>
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <DetailMetric
                      label="Submitted"
                      value={formatDate(activeSubmission.submittedAt)}
                    />
                    <DetailMetric
                      label="Maker"
                      value={activeSubmission.makerName ?? "Unassigned"}
                    />
                    <DetailMetric
                      label="Checker"
                      value={activeSubmission.checkerName ?? "Unassigned"}
                    />
                    <DetailMetric
                      label="Attachments"
                      value={String(
                        activeSubmission.answerPayload?.attachments?.length ??
                          0,
                      )}
                    />
                  </div>
        ...
        <div className="space-y-3">
          {submission.questions.length === 0 ? (
            <p className="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground">
              No question-level answer was stored for this assessment.
            </p>
          ) : (
            submission.questions.map((question, index) => (
              <article
                key={question.questionId}
                className="min-w-0 rounded-xl border border-border bg-muted/15 p-4"
              >
                ...
                <p className="mt-3 whitespace-pre-wrap break-words rounded-lg bg-background p-3 text-sm text-muted-foreground">
                  {question.learnerAnswer ?? "No inline answer submitted."}
                </p>
              </article>
            ))
          )}
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-2xl border border-emerald-200/80 bg-card shadow-sm dark:border-emerald-900/80">
      <div className="border-b border-emerald-200/80 bg-emerald-50/70 px-5 py-4 dark:border-emerald-900/80 dark:bg-emerald-950/30">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Maker and checker review shown together
        </p>
      </div>
      <div className="space-y-4 p-5">
        <div className="grid gap-3 sm:grid-cols-2">
          <DetailMetric
            label="Maker Marks"
            value={
              submission.makerTotalMarks === null
                ? "Pending"
                : `${submission.makerTotalMarks}/${submission.totalMarks}`
            }
          />
          <DetailMetric
            label="Checker / Final"
            value={displayScore(submission)}
          />
          <DetailMetric
            label="Maker Updated"
            value={formatDate(submission.makerMarkedAt)}
          />
          <DetailMetric
            label="Checker Updated"
            value={formatDate(submission.checkedAt)}
          />
        </div>
        {submission.returnReason ? (
          <Feedback
            label="Return reason"
            value={submission.returnReason}
            tone="rose"
          />
        ) : null}
        {submission.makerComment ? (
          <Feedback
            label="Maker comment"
            value={submission.makerComment}
            tone="blue"
          />
        ) : null}
        {submission.checkerComment ? (
          <Feedback
            label="Checker comment"
            value={submission.checkerComment}
            tone="emerald"
          />
        ) : null}
        <div className="space-y-3">
          {submission.questions.length === 0 ? (
            <div className="rounded-xl border border-border p-4 text-sm">
              ...
              <p className="mt-1 text-muted-foreground">
                Final score: {displayScore(submission)}
              </p>
            </div>
          ) : (
            submission.questions.map((question, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <p className="text-sm font-bold text-emerald-600">
                    {finalQuestionMarks(question) ?? "—"}/{question.maxMarks}
                  </p>
                </div>
                ...
                {question.makerComment ? (
                  <p className="mt-3 text-xs text-muted-foreground">
                    Maker: {question.makerComment}
                  </p>
                ) : null}
                {question.checkerComment ? (
                  <p className="mt-1 text-xs text-muted-foreground">
                    Checker: {question.checkerComment}
                  </p>
                ) : null}
              </article>
            ))
...
function Feedback({
  ...
  return (
    <div className={`rounded-xl border p-4 text-sm ${color}`}>
...
function SummaryCard({
  ...
  return (
    <div
      className={`flex items-center justify-between rounded-2xl border p-4 shadow-sm ${color}`}
    >
      <div>
      ...
      <span className="rounded-xl bg-white/60 p-3 dark:bg-black/15">
