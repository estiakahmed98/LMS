...
import ActivityFilterTypeahead from "@/components/admin/ActivityFilterTypeahead";
import { EntryRow, StatCard, formatActionLabel, groupByDay } from "@/components/admin/activity-log-shared";
...
export default function ActivityLogPage() {
  ...
  const fetchActorOptions = useCallback(async (search: string) => {
    const params = new URLSearchParams();
    if (search) params.set("search", search);
    const response = await fetch(`/api/admin/activity-log/actors?${params.toString()}`, {
      cache: "no-store",
    });
    if (!response.ok) return [];
    const json: { actors: AdminActivityActor[] } = await response.json();
    return json.actors;
  }, []);

  const fetchEntityOptions = useCallback(async (search: string) => {
    const params = new URLSearchParams();
    if (search) params.set("search", search);
    const response = await fetch(`/api/admin/activity-log/entities?${params.toString()}`, {
      cache: "no-store",
    });
    if (!response.ok) return [];
    const json: { entities: string[] } = await response.json();
    return json.entities;
  }, []);

  const fetchActionOptions = useCallback(async (search: string) => {
    const params = new URLSearchParams();
    if (search) params.set("search", search);
    const response = await fetch(`/api/admin/activity-log/actions?${params.toString()}`, {
      cache: "no-store",
    });
    if (!response.ok) return [];
    const json: { actions: string[] } = await response.json();
    return json.actions;
  }, []);
      ...
      <div className="space-y-6 p-6">
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
            ...
            <ActivityFilterTypeahead
              placeholder="Search users…"
              allLabel={label("filters.allActors", "All Users")}
              selectedLabel={actorId === "all" ? null : actorLabel}
              fetchOptions={fetchActorOptions}
              getKey={(option) => option.id}
              getLabel={(option) => option.name}
              onSelect={(option) => {
                setActorId(option?.id ?? "all");
                setActorLabel(option?.name ?? null);
              }}
            />

            <ActivityFilterTypeahead
              placeholder="Search modules…"
              allLabel={label("filters.allEntities", "All Modules")}
              selectedLabel={entity === "all" ? null : entity}
              fetchOptions={fetchEntityOptions}
              getKey={(option) => option}
              getLabel={(option) => option}
              onSelect={(option) => setEntity(option ?? "all")}
            />

            <ActivityFilterTypeahead
              placeholder="Search actions…"
              allLabel={label("filters.allActions", "All Actions")}
              selectedLabel={action === "all" ? null : formatActionLabel(action)}
              fetchOptions={fetchActionOptions}
              getKey={(option) => option}
              getLabel={(option) => formatActionLabel(option)}
              onSelect={(option) => setAction(option ?? "all")}
