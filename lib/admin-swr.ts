...
export async function cachedAdminRequest<T>(
  ...
  if (state?.data !== undefined && Date.now() - timestamp < maxAge)
    return state.data;
...
export async function adminFetch(input: RequestInfo | URL, init?: RequestInit) {
  ...
  if (
        ...
        : /\/api\/learner\/dashboard(\/|\?|$)/.test(key)
          ...
          : /\/api\/learner\/assessments(\/|\?|$)/.test(key)
            ...
            : /\/api\/learner\/courses(\/|\?|$)/.test(key)
              ? 30_000
              : /\/api\/learner\/(profile|certificates|question-bank)(\/|\?|$)/.test(
                    key,
                  )
                ? 300_000
                : /\/(activity-log|grading|submissions|notifications)(\/|\?|$)/.test(
                      key,
                    )
                  ? 10_000
    ...
    try {
      ...
      const cached = await cachedAdminRequest<CachedResponse>(
        `http:${key}`,
        async () => {
      ...
      if (error instanceof AdminFetchResponseError)
        return responseFromCache(error.response);
      throw error;
    }
  }
...
export async function learnerFetch(
  ...
  const isDynamicGet =
    ...
    (url.pathname === "/api/learner/dashboard" ||
      ...
      /^\/api\/learner\/assessments\/[^/]+(?:\/attachments)?$/.test(
        url.pathname,
      ));
...
export async function invalidateAdminSWR(...prefixes: string[]) {
  for (const key of cachedAt.keys()) {
    ...
    if (prefixes.some((prefix) => requestKey.startsWith(prefix)))
      cachedAt.delete(key);
  }
  await mutate(
    (key) => {
