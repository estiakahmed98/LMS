...
import { isExactMcqAnswer } from "@/lib/assessment-mcq";
import type {
...
async function listMcqRows(courseIds: string[], cutoff: Date, pageSize: number, skip: number) {
  ...
  const [submissions, total] = await Promise.all([
    prisma.submission.findMany({ where, select: { id: true, assessmentId: true, status: true, obtainedMarks: true, submittedAt: true, answerSheetUrls: true, user: { select: { name: true, email: true } }, assessment: { select: { title: true, courseId: true, totalMarks: true, passingMarks: true, course: { select: { title: true } }, questions: { select: { id: true, correctAnswer: true, correctAnswers: true } } } } }, orderBy: [{ submittedAt: "desc" }, { id: "desc" }], skip, take: pageSize }),
    prisma.submission.count({ where }),
  ]);
  const rows: AdminMcqResultRow[] = submissions.map((item) => { const payload = decodeAssessmentSubmissionPayload(item.answerSheetUrls); const answers = payload?.kind === "MCQ" ? payload.answers ?? {} : {}; const correct = item.assessment.questions.filter((question) => isExactMcqAnswer(answers[question.id], question.correctAnswers, question.correctAnswer)).length; return { id: item.id, assessmentId: item.assessmentId, assessment: item.assessment.title, courseId: item.assessment.courseId, course: item.assessment.course.title, student: item.user.name, email: item.user.email, obtainedMarks: item.obtainedMarks, totalMarks: item.assessment.totalMarks, passingMarks: item.assessment.passingMarks, scorePercent: item.obtainedMarks === null || !item.assessment.totalMarks ? null : Math.round(item.obtainedMarks / item.assessment.totalMarks * 100), passed: item.obtainedMarks === null ? null : item.obtainedMarks >= item.assessment.passingMarks, answered: Object.keys(answers).length, correct, questionCount: item.assessment.questions.length, status: item.status, submittedAt: item.submittedAt?.toISOString() ?? null }; });
  return { rows, total };
}
